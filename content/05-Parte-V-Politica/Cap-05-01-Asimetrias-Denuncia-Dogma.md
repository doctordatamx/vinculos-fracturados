...
flowchart LR
  subgraph ES [España]
    LOPIVI[LOPIVI 2021 veto SAP]
    AP2026[Anteproyecto 2026]
    LOPIVI --> AP2026
  end
  subgraph MX [México]
    SCJN[SCJN ponderación]
    DOF[DOF 2024 interpósita]
    Prot[Protocolos CDMX/Edomex]
    SCJN --> Prot
    DOF --> Prot
  end
  ES -.->|comparar sin copiar| MX
```
...
flowchart TB
  subgraph Capas
    L[LAMVLV vicaria ~30]
    P[Penal vicaria 22]
    R[Protocolo PJ / tribunal]
  end
  L --> Asim[Asimetría operativa por estado]
  P --> Asim
  R --> Asim
  Asim --> Meland[Hechos caso a caso — no el rótulo del estado]
```

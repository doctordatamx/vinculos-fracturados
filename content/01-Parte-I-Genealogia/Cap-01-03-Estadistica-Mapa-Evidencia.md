---
title: "Mapa estadístico: prevalencia, base rates y límites de la evidencia"
...
```mermaid
xychart-beta
    title "Hine UK: umbrales de exposición a PABs (n=1004, 18-25 años)"
    x-axis [">=1 PAB", ">=10 PABs", ">=20 PABs"]
    y-axis "% encuestados" 0 --> 100
    bar [98, 55, 25]
```
...
```mermaid
xychart-beta
    title "CIS-98: % alegaciones intencionalmente falsas"
    x-axis ["Todas las investigaciones", "Con disputa custodia/acceso"]
    y-axis "%" 0 --> 20
    bar [4, 12]
```
...
```mermaid
pie showData
    title "Faller 1996 (N=215): lectura simplificada substanciacion clinica"
    "Substanciadas (~73%)" : 73
    "No / otras (~27%)" : 27
```
...
```mermaid
xychart-beta
    title "SRE Mexico: solicitudes anuales (casos)"
    x-axis [2008, 2010, 2012, 2014, 2016, 2018, 2020, 2022, 2024]
    y-axis "Casos" 0 --> 400
    line [278, 222, 347, 295, 264, 225, 169, 217, 319]
```
...
```mermaid
xychart-beta
    title "ENDIREH 2021: prevalencias clave MX (mujeres >=15)"
    x-axis ["Vida (cualquier)", "12 meses", "Pareja psic. vida", "Pareja psic. 12m", "Sep/div/viu vida"]
    y-axis "%" 0 --> 100
    bar [70, 43, 35, 18, 74]
```
...
```mermaid
xychart-beta
    title "ED 2024 microdatos: sexo del custodio (custodia a una persona)"
    x-axis ["Mujer", "Hombre"]
    y-axis "%" 0 --> 100
    bar [87, 13]
```
...
```mermaid
xychart-beta
    title "ED 2024: % mujer beneficiaria (hetero; custodia vs pension adulto)"
    x-axis ["Custodia una persona", "Pension adulto"]
    y-axis "%" 0 --> 100
    bar [87, 59]
```
...
```mermaid
flowchart TD
  Q[¿Régimen de convivencia?] --> S{¿Seguridad / IPV / abuso?}
  S -->|Sí riesgo| Lim[Limitar / supervisar / unilateral protectora]
  S -->|No| C{¿Calidad parental mínima en ambos?}
  C -->|No| Unil[Unilateral o tiempo asimétrico + apoyo]
  C -->|Sí| K{¿Conflicto arrastra al menor?}
  K -->|Crónico/triangulación| HC[Alto conflicto: Mahrer mixta — no forzar 50/50]
  K -->|Manejable| F{¿Factible logística/edad?}
  F -->|Sí| SPC[Compartida / shared ≥~30-35% pertinente]
  F -->|No| Asim[Asimétrica viable + contacto significativo]
```
...
```mermaid
flowchart LR
  Q1[¿Hay interferencia medible?] --> Hine[PABs Hine UK]
  Q2[¿Hay denuncias falsas?] --> Trocme[CIS-98 4% / 12%]
  Q2 --> Smit[Smit ~1/7-8 CSA]
  Q3[¿Quién sustrae internacional?] --> HCCH[HCCH 75% madres]
  Q4[¿Cuánto MX La Haya?] --> SRE[SRE + US State]
  Q5[¿IPV mujeres MX?] --> ENDIREH[ENDIREH 70%/43%]
  Q6[¿Volumen divorcio MX?] --> ED[ED 2024 162k]
  Q7[¿PABs / vicaria MX?] --> Gap[Gap ENDIREH 2026]
  Q8[¿LATAM cercano?] --> CL[Portilla CL n=416]
  Q9[¿Compartida vs unilateral?] --> SPC[Bauserman/Nielsen/Vowels — condicional]
  Hine --> Adv[Tasas no son veredicto]
  SPC --> Adv
  Trocme --> Adv
  SRE --> Adv
  ENDIREH --> Adv
...
<!-- sync:mermaid-boot -->
<script type="module">
/* Quartz OFM mermaid solo escucha nav/render; en carga directa el SVG no se pinta. */
(async () => {
  const ready = () => {
  ...
  if (nodes.every((n) => n.querySelector("svg"))) return;
  const sources = new WeakMap();
  for (const n of nodes) sources.set(n, n.innerText);

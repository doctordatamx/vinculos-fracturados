---
title: "Jurisprudencia mexicana (SCJN): custodia, violencia familiar e interés superior"
...
```mermaid
xychart-beta
    title "SRE México: solicitudes anuales de restitución"
    x-axis [2008, 2009, 2010, 2011, 2012, 2013, 2014, 2015, 2016, 2017, 2018, 2019, 2020, 2021, 2022, 2023, 2024]
    y-axis "Conteo" 0 --> 520
    line "Casos" [278, 248, 222, 300, 347, 324, 295, 314, 264, 258, 225, 261, 169, 234, 217, 289, 319]
    line "Menores" [432, 371, 316, 444, 494, 487, 421, 474, 397, 381, 340, 354, 236, 305, 306, 379, 443]
```
...
```mermaid
xychart-beta
    title "SRE: casos activos (stock) — casos vs menores"
    x-axis ["Desde MX", "Hacia MX"]
    y-axis "Conteo" 0 --> 420
    bar "Casos" [291, 187]
    bar "Menores" [385, 241]
```
...
```mermaid
pie showData
    title SRE: cierres 2025 por motivo (casos)
    "Cierre admin. AC requirente" : 51
    "Cierre admin. AC requerida" : 25
    "Restitución voluntaria" : 24
    "Restitución judicial" : 22
    "Restitución negada" : 13
    "Acuerdo judicial" : 12
    "Otro" : 11
    "Desistimiento" : 8
    "Visitas judiciales" : 4
    "Acuerdo extrajudicial" : 3
```
...
```mermaid
xychart-beta
    title "Desde México → país destino (menores, stock)"
    x-axis ["EE.UU.", "Canadá", "Colombia", "Rusia", "Guatemala", "Irlanda", "España", "Francia", "Honduras", "Italia"]
    y-axis "Menores" 0 --> 320
    bar [310, 12, 8, 6, 4, 4, 3, 3, 3, 3]
```
...
```mermaid
xychart-beta
    title "Hacia México ← país procedencia (menores, stock)"
    x-axis ["EE.UU.", "Honduras", "España", "Venezuela", "Colombia", "Canadá", "Alemania", "Chile", "Nicaragua", "R. Unido"]
    y-axis "Menores" 0 --> 110
    bar [98, 23, 14, 13, 11, 9, 7, 7, 6, 6]
```
...
```mermaid
pie showData
    title "HCCH 2021: persona que sustrae (global)"
    "Madres" : 75
    "Padres" : 23
    "Otros" : 2
```
...
```mermaid
xychart-beta
    title "HCCH: % madres vs padres en estudios sucesivos"
    x-axis [1999, 2003, 2008, 2015, 2021]
    y-axis "%" 0 --> 100
    line "Madres %" [69, 68, 69, 73, 75]
    line "Padres %" [31, 29, 28, 24, 23]
```
...
```mermaid
pie showData
    title "HCCH 2021: rol de cuidado del taking person"
    "Cuidado conjunto" : 73
    "Primario exclusivo" : 15
    "No primario" : 12
```
...
```mermaid
flowchart TB
  subgraph comunes [Rasgos comunes OJJDP]
    A[Niega valor del otro progenitor]
    B[Hijo muy pequeño]
    C[Red de apoyo]
    D[No se ve como delito]
  end
  subgraph perfiles [Seis perfiles]
    P1[1 Amenaza previa]
    P2[2 Creencia abuso + red]
    P3[3 Delirante]
    P4[4 Sociópata]
    P5[5 Intercultural / extranjero]
    P6[6 Alienado del sistema]
  end
  comunes --> perfiles
  P5 -->|La Haya| H[Traslado internacional]
  P2 -->|a veces| IPV[Huida IPV]
  P6 -->|subgrupo| IPV
```
...
```mermaid
flowchart TD
  A[Oposición al retorno: art. 13.1.b] --> B{Aserciones detalladas?}
  B -->|No| R1[Retorno: umbral no alcanzado]
  B -->|Sí| C[Evaluar prueba: abuso niño / exposición IPV / otra]
  C --> D{Grave risk establecido?}
  D -->|No| R1
  D -->|Sí| E{Medidas protectoras eficaces y ejecutables?}
  E -->|Sí| R2[Retorno con protecciones]
  E -->|No| R3[Excepción: no ordenar retorno]
```

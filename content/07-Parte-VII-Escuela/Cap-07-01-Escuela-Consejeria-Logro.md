...
```mermaid
flowchart TD
  A[Progenitor solicita retiro / info / visita en escuela] --> B{Hay resolución judicial vigente?}
  B -->|No| C[Aplicar registro de autorizados / info a ambos]
  B -->|Sí| D[Ejecutar sentencia / medida de protección]
  D --> E[No reinterpretar: no diagnosticar PA]
  C --> E
  E --> F[Observar cambios / derivar consejería]
  F --> G[No usurpar peritaje ni juzgado]
```
